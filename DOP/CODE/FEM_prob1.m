...
a = 1/(3*h)*[7 -8 1; -8 16 -8; 1 -8 7];          %%%% Local stiffness matrix %%%%


for i=1:2:2*N-1
    phi1 = @(x)(x-X(i+1))*(x-X(i+2))/(h^2/2);    %%%% linear basis function %%%%
    phi2 = @(x)(x-X(i))*(x-X(i+2))/(-h^2/4);     %%%% linear basis function %%%%
    phi3 = @(x)(x-X(i))*(x-X(i+1))/(h^2/2);
